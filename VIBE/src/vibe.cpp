...
void VIBE::init(const cv::Mat &img)
{
    ...
    model_ = new unsigned char[chanels_ * samples_ * size_.width * size_.height];
    ...
    for (int i = 0; i < img.rows; i++)
    {
        for (int j = 0; j < img.cols; j++)
        {
            ...
            {
                model_[chanels_ * samples_ * size_.width * i + chanels_ * samples_ * j + c] = image[chanels_ * size_.width * i + chanels_ * j + c];
            }
            for (int s = 1; s < samples_; s++)
            {
                cv::Vec2i rnd_pos = getRndNeighbor(i, j);
                int img_idx = chanels_ * size_.width * rnd_pos[0] + chanels_ * rnd_pos[1];
...
void VIBE::update(const cv::Mat& img)
{
    ...
    for (int i = 0; i < img.rows; i++)
    {
        for (int j = 0; j < img.cols; j++)
        {
            ...
            int img_idx = chanels_ * size_.width * i + chanels_ * j;
